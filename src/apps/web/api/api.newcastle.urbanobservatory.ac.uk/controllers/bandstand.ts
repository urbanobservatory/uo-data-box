...
export class BandstandController {
  ...
  static async processUOTimeseries(timeseries): Promise<any> {
    const timeWindows: number[] = Object.keys(timeseries)
      .map((t) => parseInt(t))
      .sort();
    const lowTime = new Date(timeWindows[0] * 1000);
    const highTime = new Date((timeWindows.slice(-1).pop() + 3600) * 1000);
      ...
      Object.keys(bucket).forEach((timestamp: string) => {
        ...
        const meanValue =
          variable === "Traffic Flow"
            ? valueSet.reduce((s, v) => s + v, 0)
            : valueSet.reduce((s, v) => s + v, 0) / valueSet.length;
  ...
  static async processUSBTimeseries(timeseries): Promise<any> {
    const usbTimeseries = [
    ...
    Object.keys(energyBuckets).forEach((idx: string) => {
      ...
      Object.keys(bucket).forEach((timestamp: string) => {
        const valueSet = bucket[timestamp];
        const meanValue = valueSet.reduce((s, v) => s + v, 0) / valueSet.length;
        if (
  ...
  static async processUOV2Timeseries(timeseries): Promise<any> {
    const uoTimeseries = [
    ...
    Object.keys(apiBuckets).forEach((idx: string) => {
      ...
      Object.keys(bucket).forEach((timestamp: string) => {
        ...
        const meanValue =
          (valueSet.reduce((s, v) => s + v, 0) || 0) / (valueSet.length || 1);
