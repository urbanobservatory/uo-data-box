...
const lastClassifications: {[id: string]: Classification} = loadFromCacheFile('image-classifier.json');
    ...
    child_process.exec(
      ...
      (error: Error, stdout: string, stderr: string) => {
        if (error) {
          ...
          if (error.message.indexOf('FileNotFound') >= 0 &&
        ...
        const assignedClass = parseInt(stdout, 10);
...
export function trainImage(sourceId: string) {
  ...
  trainingQueue.addQueue(() => new Promise((resolve: Function, reject: Function) => {
    ...
    const trainingDates = [
      ...
      return ('0000' + d.getFullYear()).substr(-4) +
    ...
    child_process.exec(
      `python3 "/data/cam2views/create_views_classifier.py" -m ${trainingFile} ` +
        `-i ${trainingSources.join(' ')}${knownClusters ? ` -c ${knownClusters}` : ''}`,
      (error: Error, stdout: string, stderr: string) => {
        let assignedCount = knownClusters || parseInt(stdout, 10);
