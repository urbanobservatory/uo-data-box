...
export class WebsocketServerClient {
  ...
  constructor (ws: WebSocket, request: HTTP.IncomingMessage, parentServer: WebsocketServer) {
    this.socket = ws;
    this.remoteAddress = request.connection.remoteAddress;
    this.remotePort = request.connection.remotePort;
    this.server = parentServer;
    this.addEventListeners();
    log.verbose(`New WebSocket connection established with ${this.remoteAddress}:${this.remotePort}.`);
    this.send({
  ...
  private handleDisconnect() {
    log.verbose(`Connection over Websocket from ${this.remoteAddress}:${this.remotePort} was closed.`);
  ...
  public close() {
    log.verbose(`Closing a WebSocket connection to ${this.remoteAddress}:${this.remotePort}...`);
    this.send({
