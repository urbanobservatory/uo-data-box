...
export class WebsocketServer {
  ...
  private addEventListeners() {
    events.on('app:end:*', () => {
      ...
      log.info(`Received app termination notification. Should terminate Websocket connections...`);
      this.stopListening();
    });
  }
  ...
  public async startListening() {
    if (this.instance) {
      log.warn('Websocket server already exists. Attempting to destroy first...');
      await this.stopListening();
    }
    return new Promise((resolve: Function, reject: Function) => {
      log.info(`Creating new Websocket server to listen on port ${this.options.port || 8080}...`);
