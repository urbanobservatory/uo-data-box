...
export class FileCache {
  ...
  public startMonitoring() {
    ...
    this.writeTimer = setInterval(
      () => this.writeFile(),
      (this.options.saveFrequency || Number.parseInt(Config.getValue('file_cache_frequency'), 10) || 10000) * 1000
    );
  ...
  private readonly appEndHandler = () => {
    if (!this.writeTimer) return;
    log.verbose(`Received app termination notification. Should discontinue file caching (FN: ${this.filename})...`);
    this.writeFile();
    this.endMonitoring();
  };
  ...
  public setFilename(fn: string) {
    this.filename = fn;
    if (this.filename && this.options.autoStart === undefined || this.options.autoStart) {
      this.startMonitoring();
      this.fileExists = fs.existsSync(`./${this.options.directory || 'cache'}/${this.filename}.${this.options.compress ? 'lzw': 'json'}`);
    }
  }

  public async writeFile() {
    if (!this.filename) {
      return Promise.reject(new Error('No filename has yet been assigned for the cache file.'));
    }

    return new Promise((resolve: Function, reject: Function) => {
      const filePath = `./${this.options.directory || 'cache'}/${this.filename}.${this.options.compress ? 'pack' : 'json'}`;
      ...
      fs.writeFile(
        ...
        (e: Error) => {
          if (!e) {
            log.verbose(`Successfully stored cache data for '${this.filename}' to file.`);
      ...
      fs.readFile(
        ...
        (e: Error, data: string) => {
          if (!e) {
            ...
            log.warn(`Failed to read file-based cache data from '${this.filename}'.`);
            log.warn(`  ${e.message}`);
            log.debug(`  ${e.stack}`);
            resolve([]);
          }
