...
export class Data extends StorageBase {
  ...
  static lastFullUpdate: Date = null;
  ...
  protected static addEventListeners() {
    events.on('app:end:*', async () => {
      ...
      log.info(`Received app termination notification. Should flush buffer for '${this.tableName}'...`);
      await this.flushRecords(false);
      log.info(`Completed flushing buffers for '${this.tableName}'.`);
    });
  }

  public static async flushRecords(singular: boolean, asyncCommit: boolean = false) {
    ...
    if (flushData.length > 0) {
      log.warn(`${flushData.length} records are required to be committed from ${this.tableName}.`);
    }

    if (singular) {
      log.verbose(`Falling back to ${flushData.length} singular inserts to ${this.tableName}.`);
      for (const insert of flushData) {
        if (insert.value === undefined ||
            !insert.time ||
      ...
      await transaction(this.knex(), async (trx) => {
        await trx.schema.raw(
          flushData.map((insert: DataProperties) =>
            trx.raw(`INSERT INTO "${this.tableName}" ("duration", "time", "timeseries_num", "value") VALUES (?, ?, ?, ?);`,
            [
              insert.duration,
              insert.time,
              insert.timeseries_num,
              Object(insert.value) === insert.value ? JSON.stringify(insert.value) : insert.value
            ])
          ).join(' ')
        ).catch((e: any) => {
    ...
    log.verbose(`Insert batch complete. ${this.insertBuffer.length} records remain buffered.`);
    if (requireSingular) {
      log.verbose('Errors were encountered during bulk insert, will retry as singular.');
  ...
  public static getMostRecent(timeseriesId: string) {
    ...
    return cache.fetch(`Get most recent data for '${timeseriesId}'`, 3600 * 24 * 7);
  }

  public static async getHistoric(timeseriesNum: number, startTime: Date, endTime: Date): Promise<DataHistoric> {
    // TODO: Allow the limit to be customised
    log.verbose(`Requesting historic timeseries '${timeseriesNum}' from '${startTime.toISOString()}' to '${endTime.toISOString()}'`);
  ...
  public static async queryAllMostRecent() {
    ...
    const tsSince: Date = this.lastFullUpdate || new Date(Date.now() - 2 * 24 * 3600 * 1000);
    ...
    const tsRecords = await (Timeseries.query()
      ...
      `)).catch((e: Error) => {
        log.info(`Failed to update most recent values from '${this.tableName}'.`);
        log.info(`  ${e.message}`);
        log.debug(e.stack);
       }));
    if (!tsRecords) return;
    tsRecords.forEach(
      (record: any) => {
        cache.commit(
          `Get most recent data for '${record.timeseriesId}'`,
          record
        );
      }
    );
