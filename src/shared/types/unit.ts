...
export class Unit extends StorageBase {
  ...
  public static async assert(u: UnitProperties, trx?: Transaction, instance?: Unit): Promise<any> {
  ...
  public static async getById(unitId: string, trx?: Transaction): Promise<Unit> {
    return await this.namedQuery(`Get unit with the ID '${unitId}'`, trx)
      .findOne({unit_id: unitId});
  }

  public static async getByName(name: string, trx?: Transaction): Promise<Unit> {
    return await this.namedQuery(`Get unit from name '${name}'`, trx)
      .findOne({name: name});
  }
