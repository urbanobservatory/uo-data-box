...
export class Broker extends StorageBase implements BrokerProperties {
  ...
  public static async assert(p: BrokerProperties, trx?: Transaction, instance?: Broker): Promise<any> {
  ...
  public static async create(p: BrokerProperties, trx?: Transaction): Promise<any> {
  ...
  public static async getById(brokerId: string, trx?: Transaction): Promise<Broker> {
    return await this.namedQuery(`Get broker with the ID '${brokerId}'`, trx)
      .findOne({broker_id: brokerId});
  }

  public static async getByName(name: string, trx?: Transaction) {
    return await this.namedQuery(`Get broker with the name '${name}'`, trx)
      .findOne({name});
  }
}
