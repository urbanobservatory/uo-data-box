...
export class SocketController extends Controller {
  ...
  constructor (options: SocketControllerOptions) {
    ...
    this.options.values.forEach((v: SocketValueDescription) => {
      if (v.matrix) {
        this.expectedLength += v.matrix.reduce((a: number, v: number) => a *= v, 1);
      } else if (v.length) {
    ...
    log.verbose(`Expecting ${this.options.name} to provide ${this.expectedLength} values.`);
  }
  ...
  public async connect(): Promise<any> {
    ...
    return new Promise((resolve: Function, reject: Function) => {
      ...
      this.connection.on('data', (d: Buffer) => {
        ...
        if (this.receiveBuffer.length && ['\n', '\r', String.fromCharCode(3)].indexOf(this.receiveBuffer.substr(-1)) >= 0) {
          if (this.options.acquisition.stripCharacters) {
            this.receiveBuffer = this.receiveBuffer.replace(/[^A-Z0-9.; ,]/ig, '');
          }
          const dataItems = this.receiveBuffer.split(this.options.acquisition.delimiter);
  ...
  private interpretStream(values: string[]) {
    ...
    if (values.length !== this.expectedLength) {
      log.warn(`Received socket data that does not match expected length from ${this.options.name}.`);
      return {};
    }

    this.options.values.forEach((v: SocketValueDescription) => {
      const castValues = (set: string[]) => set.map((s: string) => {
        switch (v.castAs) {
          case 'string':
            return s.trim();
          case 'float':
      ...
      if (v.matrix) {
        ...
        streamValues[v.name] = toMatrix(
          castValues(values.splice(0, v.matrix.reduce((a: number, v: number) => a *= v, 1))),
          v.matrix[0]
        );
      } else if (v.length) {
    ...
    Object.keys(streamData).forEach((k: string) => {
      ...
      if (!this.variables[k]) {
        this.variables[k] = new SocketVariable({
          connection: this,
          identifier: k,
          description: this.options.values.find((v: SocketValueDescription) => v.name === k)
        });
      }
