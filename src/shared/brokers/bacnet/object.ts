...
import {BACNETObjectTypes, BACNETPropertyData, BACNETPropertyIdentifier,
        BACNETPropertyResponse, BACNETTagIDToName} from './enums';
...
export class BACNETObject extends Datapoint {
    ...
    return {
      ...
      timeseries: {
        ...
        value: {
          ...
          timeAccuracy: (this.lastSeen.getTime() - this.previousSeen.getTime()) / 1000,
  ...
  public requestMetadata() {
    log.info(`Discovered BACNET ${BACNETObjectTypes[this.options.type]} '${this.options.name}' (ID: ${this.options.identifier}) on ${this.connection.getName()}.`);
    log.verbose(`Requesting additional metadata for BACNET object '${this.options.name}'...`);
  ...
  public requestProperties(properties: BACNETPropertyIdentifier[]) {
    this.connection.getProperties({
      ...
      log.debug(`Received property response data for BACNET object '${this.options.name}'...`);
      this.consumePropertyData(objectData);
    }).catch((error: Error) => {
      log.error(`Error during property discovery on BACNET object ${this.options.name}.`);
  ...
  public consumePropertyData(objectData: BACNETPropertyResponse) {
    const minimumCOVInterval = parseInt(Config.getValue('minimum_cov_interval') || '10', 10);
    ...
    if (objectData.objectIdentifier !== this.options.identifier ||
        objectData.objectType !== this.options.type) {
      return;
    }

    const covPrecision = this.connection.getOptions().getCOVPrecision && this.connection.getOptions().getCOVPrecision({
      name: this.options.name || '',
      description: (this.properties['Description'] || {value: ''}).value,
      units: (this.properties['Units'] || {value: ''}).value
    }) || 3;

    const previousValue = this.properties['PresentValue'] ? Number(this.properties['PresentValue'].value).toPrecision(covPrecision) : undefined;
    const previousProperties = this.properties;
    this.properties = {
      ...this.properties,
      ...objectData.properties
    };
    const updatedValue = this.properties['PresentValue'] ? Number(this.properties['PresentValue'].value).toPrecision(covPrecision) : undefined;
    const objectDescription = this.properties['Description'] ? this.properties['Description'].value : 'Unknown';

    // Ensure we have a name if available
    if (this.properties['Name']) {
      this.options.name = this.properties['Name'].value;
      if (this.properties['Description'] && updatedValue !== undefined && previousValue === undefined) {
        log.verbose(
    ...
    this.lastSeen = new Date();
    if (previousValue !== undefined && previousValue !== updatedValue && this.previousSeen) {
      if (this.lastUpdated && Date.now() - this.lastUpdated.getTime() < minimumCOVInterval * 1000) {
        log.verbose(`Omitting change notification for '${this.options.name}' (${objectDescription}).`);
