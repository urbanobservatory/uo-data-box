...
export class KNXController extends Controller {
  ...
  public async connect() {
    ...
    log.verbose(`Attempting to connect to KNX controller ${this.options.name}...`);
      ...
      this.connection = new KNX.Connection(<any>{
        ...
        handlers: {
          connected: () => {
            ...
            this.connection.on('disconnected', () => {
              if (!this.disconnecting) {
                log.warn(`Unexpected disconnection from KNX controller ${this.options.name}.`);
                return;
              }
              this.connected = false;
              log.verbose(`Disconnected from KNX controller ${this.options.name}.`);
            });
          ...
          event: (evt: string, src: string, dest: string, value: Buffer) => {
            log.debug(`Received KNX [${this.options.name}]`, evt, src, dest, value);
  ...
  private trackAddress(knxAddress: string, value?: Buffer) {
    ...
    address.updateStatistics(value);
  }
