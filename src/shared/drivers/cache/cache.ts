...
export class CacheManager {
  ...
  private addPurgeTimer() {
    const nextPurge = Math.ceil(
      ...
      this.purgeInterval * 1000 - Date.now()
    );
  ...
  public fetch(key: string, expiration: number) {
  ...
  public recursivelyAssociate(key: string, entity: StorageBase | StorageBase[], child: boolean = false) {
  ...
  public associate(key: string, entity: StorageBase) {
    ...
    const uuidSet = json.match(/[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}/gi);
  ...
  public purgeByAssociation(entity: StorageBase) {
    Object.keys(this.store).forEach((key: string) => {
      if (this.store[key] && this.store[key].associations.indexOf(entity) >= 0) {
        delete this.store[key];
      }
    });
  }
