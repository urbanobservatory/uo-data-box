...
export const universalDefinitions = {
  ...
  BadRequest: {
    type: 'object',
    description:
      'An invalid ID or query parameter was used to request information, such as an ID not in the [UUID syntax](https://en.wikipedia.org/wiki/Universally_unique_identifier).',
    properties: {
      error: {
        type: 'boolean',
      ...
      code: {
        ...
        enum: ['MalformedUUID', 'BadRequestError'],
      },
    },
    example: {
      error: true,
  ...
  Link: {
    ...
    properties: {
      rel: {
        type: 'string',
        description:
          'The relation of the URL to the entity in the API, which will be taken from [this list](https://developer.mozilla.org/en-US/docs/Web/HTML/Link_types) where possible, but non-standard relations are permitted.',
      },
      href: {
        type: 'string',
      ...
      method: {
        type: 'string',
        description:
          'The HTTP protocol method to be used, or `GET` if not specified.',
        default: 'GET',
